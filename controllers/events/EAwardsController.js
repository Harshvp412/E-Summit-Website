 ...
 */
const eAwardsSubmit = async (req, res) => {
	// console.log(req.body.url);
	const { round } = req.params;
	const { _id: leaderUserID } = verifyRequest(req, "ESUMMIT_IITM_AUTH_TOKEN", ESUMMIT_SECRET);
	const { _id: eAwardsID } = await eventsModel.findOne({ name: "e-awards" });

	try {
		await participantsModel
			.updateOne(
				{ userID: leaderUserID, eventID: eAwardsID },
				{
					$addToSet: {
						submissions: {
							round: round,
...
module.exports = { eAwardsSubmit, getS3SignedPolicy };
