 ...
 */
const elympicsSubmit = async (req, res) => {
	// console.log(req.body.url);
	const { round } = req.params;
	const { _id: leaderUserID } = verifyRequest(req, "ESUMMIT_IITM_AUTH_TOKEN", ESUMMIT_SECRET);
	const { _id: elympicsID } = await eventsModel.findOne({ name: "elympics" });

	try {
		await participantsModel
			.updateOne(
				{ userID: leaderUserID, eventID: elympicsID },
				{
					$addToSet: {
						submissions: {
							round: round,
							submissionType: "url",
							name: "salesDaySubmission",
...
module.exports = { elympicsSubmit, getS3SignedPolicy };
